    ...
    return DefaultTabController(length: 2, child: Scaffold(
      ...
      body: Column(children: [
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Inventario', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
            const SizedBox(height: 2),
            const Text('Gestión de ajustes y compras', style: TextStyle(fontSize: 13, color: AppColors.textHint)),
            const SizedBox(height: 12),
            const TabBar(tabs: [Tab(text: 'Ajustes de Stock'), Tab(text: 'Registro de Compras')]),
          ]),
        ),
        Expanded(child: TabBarView(children: [_AjustesTab(), _ComprasTab()])),
      ]),
      floatingActionButton: Builder(builder: (context) {
        ...
        return FloatingActionButton.extended(
          ...
          onPressed: () { if (tabIndex == 0) { _showAjusteDialog(context, ref); } else { _showCompraDialog(context, ref); } },
        );
      }),
    ));
  }
...
class _AjustesTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return asyncData.when(
      ...
      error: (e, _) => _EmptyState(icon: Icons.tune, title: 'Ajustes de Inventario',
        subtitle: 'Registra mermas, ajustes de stock y movimientos manuales.', error: e.toString()),
      data: (ajustes) => ajustes.isEmpty
          ? const _EmptyState(icon: Icons.tune, title: 'Sin ajustes registrados',
              subtitle: 'Presiona "Nuevo Ajuste" para registrar movimientos de stock.')
          : ListView.separated(
              padding: const EdgeInsets.all(16), itemCount: ajustes.length,
                ...
                return Container(
                  ...
                  child: Row(children: [
                    Container(width: 40, height: 40,
                      ...
                      child: Icon(isPositivo ? Icons.add_circle_outline : Icons.remove_circle_outline, color: tipoColor, size: 20)),
                    const SizedBox(width: 14),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(a['producto']?['nombre'] as String? ?? 'Producto', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                      Text(a['motivo'] as String? ?? tipo, style: const TextStyle(color: AppColors.textHint, fontSize: 12)),
                    ])),
                    Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                      Text('${isPositivo ? '+' : ''}${a['cantidad']}', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: tipoColor)),
                      Text(_formatDate(a['created_at'] as String? ?? ''), style: const TextStyle(color: AppColors.textHint, fontSize: 11)),
                    ]),
                  ]),
                );
              }),
  ...
  String _formatDate(String iso) { try { final d = DateTime.parse(iso).toLocal(); return '${d.day}/${d.month}/${d.year}'; } catch (_) { return ''; } }
}
...
class _ComprasTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return asyncData.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (e, _) => _EmptyState(icon: Icons.local_shipping_outlined, title: 'Registro de Compras',
        subtitle: 'Registra entradas de mercadería desde proveedores.', error: e.toString()),
      data: (compras) => compras.isEmpty
          ? const _EmptyState(icon: Icons.local_shipping_outlined, title: 'Sin compras registradas',
              subtitle: 'Presiona "Nueva Compra" para registrar entradas de mercadería.')
          : ListView.separated(
              padding: const EdgeInsets.all(16), itemCount: compras.length,
              separatorBuilder: (_, __) => const SizedBox(height: 8),
              itemBuilder: (_, i) {
                final c = compras[i];
                return Container(
                  ...
                  child: Row(children: [
                    ...
                    const SizedBox(width: 14),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(c['proveedor']?['nombre_empresa'] as String? ?? 'Proveedor', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                      Text('${c['total_items'] ?? 0} ítems · Bs. ${c['total'] ?? '0.00'}', style: const TextStyle(color: AppColors.textHint, fontSize: 12)),
                    ])),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                      decoration: BoxDecoration(color: AppColors.accentGreen.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(20)),
                      child: Text(c['estado'] as String? ?? 'RECIBIDA', style: const TextStyle(color: AppColors.accentGreen, fontSize: 11, fontWeight: FontWeight.w700))),
                  ]),
                );
              }),
    );
...
class _EmptyState extends StatelessWidget {
  final IconData icon; final String title, subtitle; final String? error;
  ...
  Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(32),
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      if (error != null) ...[const SizedBox(height: 8), Text('($error)', style: const TextStyle(color: AppColors.error, fontSize: 12))],
    ])));
}
...
class _AjusteDialogState extends ConsumerState<_AjusteDialog> {
  ...
  @override
  void initState() { super.initState(); _loadProductos(); }
  ...
  Future<void> _save() async {
    ...
    try {
      await ref.read(dioProvider).post('${ApiConstants.inventario}/ajustes', data: {
        'producto_id': _productoId, 'tipo': _tipo,
        'cantidad': double.parse(_cantidadCtrl.text),
        'motivo': _motivoCtrl.text.trim().isEmpty ? null : _motivoCtrl.text.trim(),
      });
      if (mounted) { ref.invalidate(inventoryAjustesProvider); Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Ajuste registrado correctamente'))); }
    } catch (e) {
      ...
      content: SizedBox(width: 400, child: Form(key: _formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [
        if (_productos.isNotEmpty)
          DropdownButtonFormField<String>(
            ...
            items: _productos.map((p) => DropdownMenuItem(value: p['id'] as String, child: Text(p['nombre'] as String))).toList(),
        ...
        const SizedBox(height: 12),
        TextFormField(controller: _cantidadCtrl, keyboardType: const TextInputType.numberWithOptions(decimal: true),
          decoration: const InputDecoration(labelText: 'Cantidad *'),
          validator: (v) => (v == null || double.tryParse(v) == null) ? 'Ingrese una cantidad válida' : null),
        const SizedBox(height: 12),
        TextFormField(controller: _motivoCtrl, decoration: const InputDecoration(labelText: 'Motivo / Descripción'), maxLines: 2),
      ]))),
      actions: [
  ...
  Future<void> _save() async {
    ...
    try {
      await ref.read(dioProvider).post(ApiConstants.compras, data: {
        ...
        'items': [{'producto_id': _productoId, 'cantidad': double.parse(_cantidadCtrl.text), 'precio_unitario': double.parse(_costoCtrl.text)}],
      });
      if (mounted) { ref.invalidate(inventoryComprasProvider); Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Compra registrada correctamente'))); }
    } catch (e) {
      if (mounted) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e'), backgroundColor: AppColors.error)); }
    } finally { setState(() => _saving = false); }
  }
      ...
      content: SizedBox(width: 400, child: Form(key: _formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [
        if (_proveedores.isNotEmpty)
          DropdownButtonFormField<String>(
            ...
            items: _proveedores.map((p) => DropdownMenuItem(value: p['id'] as String, child: Text(p['nombre_empresa'] as String? ?? ''))).toList(),
        ...
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: TextFormField(controller: _cantidadCtrl,
            keyboardType: const TextInputType.numberWithOptions(decimal: true),
            decoration: const InputDecoration(labelText: 'Cantidad *'),
            validator: (v) => (v == null || double.tryParse(v) == null) ? 'Inválido' : null)),
          const SizedBox(width: 12),
          Expanded(child: TextFormField(controller: _costoCtrl,
            keyboardType: const TextInputType.numberWithOptions(decimal: true),
