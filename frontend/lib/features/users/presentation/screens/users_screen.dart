        ...
        Container(
          ...
          child: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Gestión de Usuarios', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
              const SizedBox(height: 2),
              asyncUsers.when(
                loading: () => const Text('Cargando...', style: TextStyle(fontSize: 13, color: AppColors.textHint)),
                error: (_, __) => const SizedBox.shrink(),
                data: (users) => Text('${users.length} usuarios registrados', style: const TextStyle(fontSize: 13, color: AppColors.textHint))),
            ])),
            ElevatedButton.icon(icon: const Icon(Icons.person_add, size: 18), label: const Text('Nuevo Usuario'),
              onPressed: () => _showCreateUserDialog(context, ref)),
          ])),
        Expanded(child: asyncUsers.when(
          ...
          error: (e, _) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            ElevatedButton.icon(icon: const Icon(Icons.refresh, size: 16), label: const Text('Reintentar'),
          ...
          data: (users) => users.isEmpty
              ...
              : ListView.separated(padding: const EdgeInsets.all(16), itemCount: users.length,
    ...
    return Container(
      ...
      child: Row(children: [
        Container(width: 44, height: 44,
          decoration: BoxDecoration(color: _roleColor(rol).withValues(alpha: 0.15), shape: BoxShape.circle),
          child: Center(child: Text(initials, style: TextStyle(color: _roleColor(rol), fontWeight: FontWeight.w700, fontSize: 15)))),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(user['nombre_completo'] as String? ?? user['username'] as String? ?? '',
            style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 15)),
          const SizedBox(height: 2),
          Text(user['email'] as String? ?? '', style: const TextStyle(color: AppColors.textHint, fontSize: 12)),
        ])),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
          decoration: BoxDecoration(color: _roleColor(rol).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(20)),
          child: Text(_rolLabel(rol), style: TextStyle(color: _roleColor(rol), fontSize: 11, fontWeight: FontWeight.w700))),
        const SizedBox(width: 10),
        Container(width: 8, height: 8,
          decoration: BoxDecoration(color: activo ? AppColors.accentGreen : AppColors.textHint, shape: BoxShape.circle)),
        const SizedBox(width: 10),
        PopupMenuButton<String>(
          ...
          itemBuilder: (_) => [
            ...
            PopupMenuItem(value: 'toggle', child: Row(children: [
              Icon(activo ? Icons.block : Icons.check_circle_outline, size: 16, color: activo ? AppColors.error : AppColors.accentGreen),
              const SizedBox(width: 8),
              Text(activo ? 'Desactivar' : 'Activar', style: TextStyle(color: activo ? AppColors.error : AppColors.accentGreen))])),
          ],
          onSelected: (v) async {
            if (v == 'edit') { showDialog(context: context, builder: (_) => _UserDialog(ref: ref, user: user)); }
            if (v == 'toggle') {
              try {
                await ref.read(dioProvider).patch('${ApiConstants.usuarios}/${user['id']}', data: {'activo': !activo});
...
class _UserDialogState extends ConsumerState<_UserDialog> {
  ...
  Future<void> _save() async {
    ...
    try {
      final body = <String, dynamic>{
        ...
        'nombre_completo': _nombreCtrl.text.trim(), 'rol': _rol,
        if (_passwordCtrl.text.isNotEmpty) 'password': _passwordCtrl.text,
      };
      if (_isEdit) { await ref.read(dioProvider).put('${ApiConstants.usuarios}/${widget.user!['id']}', data: body); }
      else { await ref.read(dioProvider).post(ApiConstants.usuarios, data: body); }
      if (mounted) {
        ref.invalidate(usersListProvider); Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(_isEdit ? 'Usuario actualizado' : 'Usuario creado correctamente')));
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: Text(_isEdit ? 'Editar Usuario' : 'Nuevo Usuario', style: const TextStyle(fontWeight: FontWeight.w700)),
      content: SizedBox(width: 420, child: Form(key: _formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [
        TextFormField(controller: _nombreCtrl, decoration: const InputDecoration(labelText: 'Nombre Completo *'),
          validator: (v) => (v == null || v.trim().isEmpty) ? 'Requerido' : null),
        const SizedBox(height: 12),
        TextFormField(controller: _usernameCtrl, decoration: const InputDecoration(labelText: 'Usuario *'),
          validator: (v) => (v == null || v.trim().isEmpty) ? 'Requerido' : null),
        const SizedBox(height: 12),
        ...
        TextFormField(controller: _passwordCtrl, obscureText: _obscure,
          decoration: InputDecoration(
            ...
            suffixIcon: IconButton(icon: Icon(_obscure ? Icons.visibility_off : Icons.visibility, size: 20),
              onPressed: () => setState(() => _obscure = !_obscure))),
          validator: (v) { if (!_isEdit && (v == null || v.isEmpty)) { return 'Requerido'; } return null; }),
      ]))),
      actions: [
