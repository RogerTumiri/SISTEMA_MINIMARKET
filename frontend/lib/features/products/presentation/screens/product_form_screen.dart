...
class _ProductFormScreenState extends ConsumerState<ProductFormScreen> {
  ...
  Future<void> _loadCatalogs() async {
    ...
    final res = await Future.wait([
      dio.get(ApiConstants.categorias),
      dio.get(ApiConstants.unidades),
    ]);
    setState(() {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_isEdit) {
        await dio.put('${ApiConstants.productos}/${widget.productId}', data: body);
      } else {
        await dio.post(ApiConstants.productos, data: body);
      }
      if (mounted) {
        context.go('/products');
